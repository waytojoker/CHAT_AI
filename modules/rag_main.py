...
import ollama
from rag_module import show_rag_management, RAGSystem
from enhanced_conversation_display import (
    display_rag_enhanced_conversation,
    show_rag_settings,
    show_rag_debug_info
)
from file_processing import get_file_content
from history_module import show_conversation_history
from model_service import create_model_service, ModelService, QianfanModelService, OllamaModelService
import os
...
def show_model_service_config():
    ...
    service_type = st.selectbox(
        "选择模型服务",
        ["ollama", "qianfan"],
        index=0 if st.session_state["model_service_type"] == "ollama" else 1,
        help="选择要使用的模型服务类型"
    )
    ...
    if service_type == "qianfan":
        ...
        authorization = st.text_input(
            "授权令牌 (Authorization)",
            value=st.session_state["qianfan_authorization"] or env_auth,
            type="password",
            help="请输入千帆API的授权令牌"
        )
        ...
        model = st.selectbox(
            "千帆模型",
            qianfan_models,
            index=qianfan_models.index(st.session_state["qianfan_model"])
            if st.session_state["qianfan_model"] in qianfan_models else 0
        )
        ...
        if st.button("测试Ollama连接"):
            try:
                service = create_model_service("ollama", host=host, model=st.session_state["ollama_model"])
...
请根据以上设定进行对话。"""
        ...
        if st.session_state["model_service_type"] == "qianfan":
            ...
            # 对于Ollama，仍使用原有的ollama客户端以支持流式输出
            client = ollama.Client(host=st.session_state["ollama_host"])
